...
static void* gpio_active_monitor_update(void* usr)
{
    ...
    while (!mon->thread_stop) {
        ...
        if (mon->currently_active && (gpio_active_monitor_active_pct(mon) < mon->falling_edge_inactive_threshold)) {
            mon->currently_active = false;
        } else if (!mon->currently_active && (gpio_active_monitor_active_pct(mon) > mon->rising_edge_active_threshold)) {
...
struct GpioPinActiveMonitor* gpio_active_monitor_create(const struct GpioPinActiveMonitor_args args) {
